...
fn make_ng_setup() -> NgSetupRequest {
	...
	NgSetupRequest {
		global_ran_node_id: GlobalRanNodeId::GlobalGnbId(GlobalGnbId {
			...
			gnb_id: GnbId::GnbId(BitString::from_inner(
				vec![0x00, 0x01, 0x02].view_bits::<Msb0>().into(),
			)),
		}),
...
#[test]
fn test_ngap_pdu_coding() -> Result<(), ThreeGppAsn1PerError> {
...
#[test]
fn test_ran_ue_ngap_id() -> Result<(), ThreeGppAsn1PerError> {
